 ...
 */
const generateRandomHex = (length: number = 16): string => {
...
/**
 * Handles image upload to the attachment system
 * @param file The image file to upload
 * @param companyId Company ID for the attachment
 * @param projectId Project ID for the attachment  
 * @param vulnerabilityId Optional vulnerability ID for the attachment
...
export const handleImageUpload = async (
  ...
  if (!companyId || !projectId) {
    throw new Error("Company ID and Project ID are required for image upload")
  }

  try {
    // Generate random 16-character hex name for images
    const fileExtension = file.name.split('.').pop() || 'png';
    const randomFileName = `image_${generateRandomHex(16)}.${fileExtension}`;

    // Create FormData for file upload to attachment system
    const formData = new FormData();
    formData.append('file', file);
    formData.append('file_name', randomFileName);
    formData.append('description', `Image uploaded from editor: ${file.name}`);
    ...
    // Return the file URL from the attachment response
    // The response structure might vary - adjust based on your API response
    const attachmentData = response.data;
    return attachmentData.file || attachmentData.url || `/api/attachments/${attachmentData.id}/file/`;
